...
namespace CastlePlus2.Client.Services.Media
{
    public class PrzylaczaService : IPrzylaczaService
    {
        ...
        public async Task<PagedResultDto<PrzylaczeLookupDto>> SearchLookupPagedAsync(
            string? q,
            int page,
            int pageSize,
            CancellationToken ct = default)
        {
            ...
            return await _http.GetFromJsonAsync<PagedResultDto<PrzylaczeLookupDto>>(url, ct)
                   ?? new PagedResultDto<PrzylaczeLookupDto>();
        }
        ...
        public async Task<bool> UpdateAsync(long id, UpdatePrzylaczeRequest request, CancellationToken ct = default)
        {
